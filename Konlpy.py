...
def get_tags(text, issues):
    ...
    count = Counter(nouns)
    return_list = []
    return_list.append((issues,count[issues]))
    return return_list

def main():
    os.makedirs("local_Konlpy")
    for i in region:
        text = open(os.path.join("region", "%s.txt" % i), encoding='utf8').read()

        open_output_file = os.path.join("local_Konlpy", "%s_Konlpy.txt" % i)
        open_output_file = open(open_output_file, "w", encoding='utf8')
        for j in water_issues:
            tags = get_tags(text, j)
            for tag in tags:
                open_output_file.write(str(tag))
